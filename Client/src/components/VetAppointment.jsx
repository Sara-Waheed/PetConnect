...
import { Video, Calendar, ThumbsUp, Star, ChevronLeft, ChevronRight, MapPin, Home } from 'lucide-react';
...
const VetAppointment = () => {
  ...
  const providerType = 'vet';
  const providerId = vetId;

  //fetch reviews
  useEffect(() => {
    // don’t even try if we’re missing data
    if (!providerType || !vetId) return;
    ...
    const fetchReviews = async () => {
      ...
      try {
        // if you’ve set axios.defaults.baseURL = 'http://localhost:5000/auth'
        // then this becomes just `/reviews/vet/${vetId}`
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        // fetch vet + services (each service has its own availability)
  ...
  const fetchSlotsForDate = async (isoDate, dayName) => {
    if (!vet || !Array.isArray(vet.services) || vet.services.length === 0) {
      return [];
    }
    ...
    try {
      ...
      // pick the first (and only) service for this vet
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-md p-6 mt-5">
        ...
        <div className="mt-8 border-t border-gray-100 pt-6">
          <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            ...
            Reviews About {vet.name.split(" ")[0]} 
