...
const EditPetAdoption = () => {
  const { id } = useParams(); // Listing ID to edit
  const navigate = useNavigate();
  
  // Multi-step state
  const [currentStep, setCurrentStep] = useState(1);
  
  // Form data state (pre-populated from backend)
  ...
  // Fetch the existing listing data on mount
  useEffect(() => {
    const fetchListing = async () => {
      try {
        ...
        setFormData({
          ...
          keepDuration: ad.keepDuration || "",
          city: ad.city || "",
          householdActivityLevel: ad.householdActivityLevel || "",
          otherPets: ad.otherPets || [],
          householdEnvironment: ad.householdEnvironment || "",
          name: ad.name || "",
          age: ad.age || "",
          breed: Array.isArray(ad.breed) ? ad.breed : [ad.breed],
          size: ad.size || "",
          gender: ad.gender || "",
          ...
          catCharacteristics: ad.catCharacteristics || [],
          rabbitCharacteristics: ad.rabbitCharacteristics || [],
          dogSocialIssues: ad.dogSocialIssues || [],
          catSocialIssues: ad.catSocialIssues || [],
          upToDateVaccinations: ad.upToDateVaccinations || "",
          upToDateFleaWorm: ad.upToDateFleaWorm || "",
          upToDateDental: ad.upToDateDental || "",
          hasHealthIssues: ad.hasHealthIssues || "",
          hasMedication: ad.hasMedication || "",
          healthDetails: ad.healthDetails || "",
          personalityDescription: ad.personalityDescription || "",
          playDescription: ad.playDescription || "",
  ...
  const handleChange = (e) => {
    ...
    setError("");
    if (name === "species") {
      setFormData((prev) => ({ ...prev, breed: [] })); // Reset breed when species changes
    }
    if (name === "phone") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/auth/update-adoption-ad/${id}`,
        submissionData,
        {
      ...
      if (response.data.success) {
        ...
        console.error("Error updating listing:", response.data.message);
        alert(`Error: ${response.data.message}`);
      }
    } catch (error) {
      console.error("Error updating listing:", error);
      alert("Failed to update adoption ad. Please try again.");
    }
  };
  ...
  const renderStep = () => {
    switch (currentStep) {
          ...
          <>
                  ...
                  return (
                    <label
                      ...
                      className={`relative w-36 h-40 flex flex-col items-center justify-center cursor-pointer transition-all
                        ...
                        ${selected ? "border-teal-600 bg-orange-100" : "border-gray-300 bg-white hover:bg-gray-50"}`}
                    >
                      <input
                        type="radio"
                        name="species"
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                {["Less than 1 month", "1 month", "2 months", "Until a home is found"].map((dur) => (
                  <label
                    key={dur}
                    className="flex items-center bg-orange-100 p-2 border border-gray-300 rounded hover:border-orange-500 cursor-pointer"
                  >
              ...
              <p className="text-sm text-gray-600 mt-2">
                To list your pet with us, you need to be able to keep your pet for a minimum of one month. We cannot deal with emergency rehoming.
              </p>
            </div>
            <div className="mb-4">
      ...
      case 2:
        return (
          <>
            <h2 className="text-2xl font-semibold text-orange-700 mb-4">Step 2: Your Household</h2>
            <div className="mb-4">
              <label className="block mb-2 font-medium">
                What best describes the household's typical activity level?<span className="text-red-600"> *</span>
              </label>
              <div className="space-y-2">
                {["Moderate comings and goings", "Quiet with occasional guests", "Busy / noisy"].map((activity) => (
                  <label
                    key={activity}
                    className="flex items-center bg-orange-100 p-2 border border-gray-300 rounded hover:border-orange-500 cursor-pointer"
                  >
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                {["Yes, my pet lives with dogs", "Yes, my pet lives with cats", "Yes, my pet lives with rabbits", "Yes, my pet lives with other small furries (such as hamsters, mice etc.)", "No, my pet lives as an ONLY pet in the household"].map((opt) => (
                  <label
                    key={opt}
                    className="flex items-center bg-orange-100 p-2 border border-gray-300 rounded hover:border-orange-500 cursor-pointer"
                  >
      ...
      case 3:
        return (
          <>
            <h2 className="text-2xl font-semibold text-orange-700 mb-4">Step 3: Pet Details</h2>
            <div className="mb-4">
            ...
            <div className="mb-4">
              <label className="block mb-2 font-medium">Size of Pet<span className="text-red-600"> *</span></label>
      ...
      case 4:
        return (
          <>
            <h2 className="text-2xl font-semibold text-orange-700 mb-4">Step 4: Describe Your Pet</h2>
                  ...
                  {rabbitCharacteristicsList.map((char) => (
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => handleCharacteristicsChange(e, "rabbitCharacteristics")}
                        className="form-checkbox h-5 w-5 accent-teal-600"
                      />
                      <span className="ml-2 text-gray-700">{char}</span>
                    </label>
      ...
      case 6:
        return (
          <>
            ...
            <div className="mb-4">
              <label className="block mb-2 font-medium">
                What foods and treats does {formData.name || "your pet"} like? Any special dietary needs?<span className="text-red-600"> *</span>
              </label>
              <textarea
      ...
      case 7:
        return (
          <>
            <h2 className="text-2xl font-semibold text-orange-700 mb-4">Final Step: Contact Info</h2>
            <div className="mb-4">
              <label className="block mb-2 font-medium">
